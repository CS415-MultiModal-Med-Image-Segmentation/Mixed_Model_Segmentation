...
\node[box=orange, above right=0.25cm and 0.7cm of rtr] (sL)
  {\textbf{Lung UNet++}\\[-1pt]EffNet-B1 + SCSE, 512$^2$};
\node[box=teal, below right=0.25cm and 0.7cm of rtr] (sB)
  {\textbf{Brain UNet++}\\[-1pt]EffNet-B3 + SCSE, 256$^2$};
\node[box=gray, right=2.9cm of rtr] (out) {\textbf{Mask}\\[-1pt]binary};
...
\caption{Anatomy-aware routing architecture. A ConvNeXt-Tiny classifier routes each input to a dedicated UNet++ specialist with SCSE attention and encoder. The mixed baseline $S^H$ shares the lung backbone (UNet++/EffNet-B1, 256$^2$) but uses no attention and is trained jointly on both organs.}
\label{fig:routing}
\end{figure}
